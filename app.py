            ...
            try:
                ...
                c.execute('''
                    ...
                    (patient_id, general_health, current_medications, allergies, headaches_history, family_history,
                     ocular_history, uploaded_reports)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?)
...
def refraction_examination():
    ...
    with st.form("subjective_form"):
        ...
        with sod_col:
            st.markdown("**Right Eye (OD)**")
            subjective_od_sphere = st.number_input("Sphere OD", value=0.0, step=0.25, format="%.2f", key="subj_od_sph2")
            subjective_od_cylinder = st.number_input("Cylinder OD", value=0.0, step=0.25, format="%.2f", key="subj_od_cyl2")
            subjective_od_axis = st.number_input("Axis OD", min_value=0, max_value=180, value=0, key="subj_od_ax2")
        ...
        with sos_col:
            st.markdown("**Left Eye (OS)**")
            subjective_os_sphere = st.number_input("Sphere OS", value=0.0, step=0.25, format="%.2f", key="subj_os_sph2")
            subjective_os_cylinder = st.number_input("Cylinder OS", value=0.0, step=0.25, format="%.2f", key="subj_os_cyl2")
            subjective_os_axis = st.number_input("Axis OS", min_value=0, max_value=180, value=0, key="subj_os_ax2")
    ...
    with st.form("final_form"):
        ...
        with left_col:
            ...
            final_od_sph = st.number_input("Final Sphere OD", value=0.0, step=0.25, format="%.2f", key="final_od_sph2")
            final_od_cyl = st.number_input("Final Cylinder OD", value=0.0, step=0.25, format="%.2f", key="final_od_cyl2")
            final_od_axis = st.number_input("Final Axis OD", min_value=0, max_value=180, value=0, key="final_od_ax2")
        with right_col:
            st.markdown("**Left Eye (OS) - Final**")
            final_os_sph = st.number_input("Final Sphere OS", value=0.0, step=0.25, format="%.2f", key="final_os_sph2")
            final_os_cyl = st.number_input("Final Cylinder OS", value=0.0, step=0.25, format="%.2f", key="final_os_cyl2")
            final_os_axis = st.number_input("Final Axis OS", min_value=0, max_value=180, value=0, key="final_os_ax2")
        # binocular and notes
        ...
        if savea:
            try:
                ...
                ''', (p['id'], biomicroscopy_od, biomicroscopy_os, biomicroscopy_notes,
                      acd_od, acd_os, acv_od, acv_os, iridocorneal_od, iridocorneal_os, pup_res, pup_res, json.dumps(pup_paths), json.dumps(file_paths)))
                conn.commit()
...
def patient_registration():
    ...
    with st.form("reg_form"):
        ...
        with c1:
            ...
            date_of_birth = st.date_input("Date of Birth*", value=date(1990,1,1), min_value=date(1900,1,1), max_value=date.today(), format=" %d.%m.%Y")

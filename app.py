...
def init_db():
    ...
    'refraction_exams': [
        ...
        'cycloplegic_type', 'final_deg_near', 'subjective_binocular_notes'
    ],
    ...
    'contact_lens_prescriptions': [
        ...
        'rgp_optical_zone', 'rgp_peripheral_curve', 'scleral_haptic', 'scleral_clearance', 
        'scleral_landing_zone', 'scleral_sagittal_depth'
    ]
}
...
''')
    ...
    CREATE TABLE IF NOT EXISTS contact_lens_prescriptions (
        ...
        scleral_haptic TEXT,
        scleral_clearance TEXT,
        scleral_landing_zone TEXT,
        scleral_sagittal_depth TEXT,
...
def refraction_examination():
    ...
    with st.form("vision_form"):
     ...
     # 3) Objective Refraction - ISPRAVLJENO: S cycloplegic opcijama
    ...
    with st.form("objective_form"):
        ...
        # Cycloplegic options - KOMPLETNO ISPRAVLJENO
        ...
        with col_cyclo1:
            ...
            if cycloplegic_used:
                cycloplegic_type = st.selectbox("Cycloplegic Type", 
                                              ["Cyclopentolate", "Atropine", "Tropicamide", "Homatropine", "Other"],
                                              key="cyclo_type")
        ...
        if submit_objective:
            st.session_state.refraction.update({
                ...
                'cycloplegic_type': cycloplegic_type if cycloplegic_used else None,
    ...
    # 6) Final Prescription WITH PRISM AND BASE - KOMPLETNO ISPRAVLJENO
    ...
    with st.form("final_form"):
        ...
        with col_final_dist_headers[7]:
            st.write("**ADD**")  # ISPRAVLJENO: BVP -> ADD
        ...
        with col_final_dist_od[7]:
            final_od_add = st.text_input("ADD OD", placeholder="e.g., +1.50", key="final_od_add", label_visibility="collapsed")  # ISPRAVLJENO
        ...
        with col_final_dist_os[7]:
            final_os_add = st.text_input("ADD OS", placeholder="e.g., +1.50", key="final_os_add", label_visibility="collapsed")  # ISPRAVLJENO
        
        # Final Near Correction - ISPRAVLJENO: DEG umjesto ADD
        ...
        with col_final_near_headers[7]:
            st.write("**DEG**")  # ISPRAVLJENO: ADD -> DEG
        ...
        with col_final_near_od[7]:
            final_near_od_deg = st.text_input("DEG OD", placeholder="e.g., 2.00", key="final_near_od_deg", label_visibility="collapsed")  # ISPRAVLJENO
        ...
        with col_final_near_os[7]:
            final_near_os_deg = st.text_input("DEG OS", placeholder="e.g., 2.00", key="final_near_os_deg", label_visibility="collapsed")  # ISPRAVLJENO
        ...
        with col_bin2:
            ...
            bvp = st.text_input("BVP", placeholder="e.g., 12.5", key="bvp")  # ISPRAVLJENO: BVP je ovdje ispod
    ...
    INSERT INTO refraction_exams (
        ...
        cycloplegic_used, cycloplegic_type, cycloplegic_agent, cycloplegic_lot, cycloplegic_expiry, cycloplegic_drops,
        ...
        final_deg_distance, final_deg_near, bvp, color_vision, prescription_notes
    ) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)
''', (
    pid, 
    ...
    final_near_os_prism, final_near_os_base,
    final_deg_distance, f"{final_near_od_deg}/{final_near_os_deg}", bvp, color_vision, prescription_notes
))
                conn.commit()
...
def contact_lenses():
    ...
    (patient_id, lens_type, lens_design, lens_material, lens_color,
     ...
     rgp_brand, rgp_base_curve, rgp_diameter, rgp_optical_zone, rgp_peripheral_curve,
     rgp_power_od_sphere, rgp_power_od_cylinder, rgp_power_od_axis, rgp_add_od,
     rgp_power_os_sphere, rgp_power_os_cylinder, rgp_power_os_axis, rgp_add_os,
     scleral_brand, scleral_diameter, scleral_haptic, scleral_clearance, scleral_landing_zone, scleral_sagittal_depth,
...
''', (p['id'], lens_type, lens_design, lens_material, lens_color,
     ...
     rgp_optical_zone if lens_type == "RGP" else None,
     rgp_peripheral_curve if lens_type == "RGP" else None,
     od_sphere, od_cylinder, od_axis, od_add,
     os_sphere, os_cylinder, os_axis, os_add,
     scleral_brand if lens_type == "Scleral" else None,
     scleral_diameter if lens_type == "Scleral" else None,
     scleral_haptic if lens_type == "Scleral" else None,
     scleral_clearance if lens_type == "Scleral" else None,
     scleral_landing_zone if lens_type == "Scleral" else None,
     scleral_sagittal_depth if lens_type == "Scleral" else None,
     od_sphere, od_cylinder, od_axis, od_add,
     os_sphere, os_cylinder, os_axis, os_add,
